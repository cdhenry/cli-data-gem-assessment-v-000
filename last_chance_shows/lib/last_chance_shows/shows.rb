...
class LastChanceShows::Shows
  ...
  attr_accessor :title, :closing, :venue, :blurb, :schedule, :run_time, :theater_url, :url

  @@all = []

  # def self.new_from_title_and_closing (title, closing)
  #   self.new
  #   @title = title
  #   @closing = closing
  # end

  def initialize (title = nil, closing = nil)
    @title = title
    @closing = closing
  end
  ...
  def make_show_url
    ...
    show_url = "http://www.playbill.com/searchpage/search?q="
    ...
    show_url.chomp("+")
    show_url = show_url + "&sort=Relevance&shows=on&qasset="
  end
